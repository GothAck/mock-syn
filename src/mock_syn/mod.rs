...
mod test {
    ...
    fn map_err(e: Error, input: &str) -> Error {
        let span = e.span();
    ...
    fn rustfmt<T: std::fmt::Display>(input: T) -> String {
        use std::{
            io::Write,
    ...
    fn test() {
        ...
        "#;
        let derive: MockSynDerive = parse_str(input).map_err(|e| map_err(e, input)).unwrap();
